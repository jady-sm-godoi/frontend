...
async function LiveReload() {
  const socketLiveReload = net.createServer((client) => {
    client.emit("data", "sss");

    const watchListener = fs.watch(
      "./src/",
      { recursive: true },
      (curr, prev) => {
        setTimeout(() => {
        ...
        console.log("Atualização detectada em", curr);
      }
    );
  ...
  socketLiveReload.on("error", () => {
    console.log("Desconectado");
  });

  socketLiveReload.listen("4001", "127.0.0.1");
}
